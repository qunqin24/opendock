...
On first run OpenCode installs the package via Bun (`~/.cache/opencode/node_modules/`) and loads it as a server plugin. The plugin registers six slash commands (`/ponytail`, `/ponytail-review`, `/ponytail-audit`, `/ponytail-debt`, `/ponytail-gain`, `/ponytail-help`), appends the bundled `skills/` to OpenCode's skill paths, and injects the ruleset into every chat's system prompt at the active level. OpenCode also auto-loads this repo's `AGENTS.md`, so the rules hold even without the plugin. Switch levels with `/ponytail lite|full|ultra|off`.
...
The package manager is **pnpm** (pinned via `packageManager`). To set up locally:
