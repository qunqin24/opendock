...
| **[Step Code Package](nowledge-mem-step-code)** | `step install npm:nowledge-mem-step-code` | Step Code-native package with startup context, `agent_settled` conversation capture, bundled memory skills, and historical backfill. |
...
| **[OpenCode Plugin](nowledge-mem-opencode-plugin)** | Check `opencode --version`, then use the [global install command](nowledge-mem-opencode-plugin#setup) for that major | Native OpenCode plugin with Context Bundle, Working Memory, search/save tools, idle-event automatic thread capture, pre-compaction flush, handoff, and status. |
...
| **[Pydantic AI Connector](nowledge-mem-pydantic-ai)** | `pip install ./nowledge-mem-pydantic-ai` from a checkout | Native capability and tools-only adapter with per-run Context Bundle, scoped MCP tools, and opt-in Thread capture. Experimental; see the package guide for supported versions. |
