...
Bounded edits outside goal mode use `osuki_review` with the actual diff, surrounding context and focused validation evidence, even when the initial complexity tier was uncertain or standard. Jev classifies scope, correctness and validation coverage. `lightweight-passed` avoids a coding-model reviewer; `evidence-required` requests focused inspection/checks or a validation-blocker report; `changes-required` requests investigation and correction of local findings. Only `reviewer-required` escalates to the configured independent reviewer. Missing evidence, an uncertain answer or an unavailable optional tool never by themselves trigger that reviewer or redundant checks; mark explicit user/repository gates as `required`, and treat their failure or unavailability as a blocker. Risky or oversized work and active goals retain independent review. Choose checks by change impact rather than running full E2E for every edit. Jev evaluates supplied evidence, does not execute tests, and never creates goal completion receipts.
