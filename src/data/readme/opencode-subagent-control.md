...
- **会话 ID 一键复制**：展开明细显示子代理 Session ID，点击 `ses_xxx… ⎘` 直接复制到系统剪贴板，支持 Windows/macOS/Linux
- **手动标记完成**：展开区右侧 `- 仅清除显示` 可手动终结卡住的僵尸条目（仅修改本地展示，不取消真实任务）；`/subagent-clear-running` 批量清理
- **真实取消**：运行中且有会话 ID 的子代理显示红色 `取消任务` 按钮，调用 OpenCode 真实取消接口，父级解除等待，最终显示 `已取消`
...
| 点击 `→ 进入会话` | 跳转子代理完整对话 |
| 点击 `ses_xxx… ⎘` | 一键复制 Session ID 到系统剪贴板（Windows/macOS/Linux/SSH） |
| 点击 `- 仅清除显示` | 仅清除插件展示，不取消真实任务（仅运行中时显示） |
| 点击 `取消任务`（红色） | 真实取消子代理，调用 session.abort，父级解除等待（需有会话 ID） |
