...
- **Two commands to set up** — `npm i -g`, then `crew install-hook` wires all three products; or install it as a plugin. Token-frugal and fail-safe by design; reads transcripts, never modifies them.

## Install

```sh
npm install -g @0xmmo/crew
crew install-hook      # wire the hook into Claude Code, Codex, and OpenCode
crew doctor            # confirm everything is connected
```

`crew install-hook` is the whole setup: it adds the hook to `~/.claude/settings.json` and `~/.codex/hooks.json`, and drops a generated crew plugin into `~/.config/opencode/plugins/`. It is idempotent and merges into settings you already have. (A global install also tries to do this from npm's postinstall, but npm hides that output and `ignore-scripts=true` skips it entirely, so don't count on it: run `crew install-hook` and let `crew doctor` tell you where you stand.) Requires Node.js ≥ 18. macOS and Linux.
...
crew doctor           # is this machine wired up? --fix repairs, --json for agents
crew --help
```
...
Every failure crew has had in the field looked healthy from the outside: hooks silently not installed because npm skipped postinstall, a global install symlinked to a working tree, a sandbox denying the mailbox, an OpenCode plugin left over from an older release. `crew doctor` checks each of those and prints the command that fixes it:

```
crew 0.4.0

✓ Node.js                   v22.14.0 at /opt/homebrew/bin/node
✓ crew on PATH              /opt/homebrew/bin/crew
✓ Claude Code hook          ~/.claude/settings.json
✗ Codex hook                not wired (npm has ignore-scripts=true, so postinstall never ran)
                            → crew install-hook
⚠ OpenCode hook             ~/.config/opencode/plugins/crew.js was generated by an older crew; re-render it
                            → crew install-hook
✓ Mailbox                   /tmp/crew-501/inbox writable
✓ Legacy inboxes            empty
✓ Session registry          ~/.crew/sessions: 3 live
✓ Claude Code sessions dir  ~/.claude/sessions: 3 record(s)
✓ OpenCode transcripts      sqlite3 3.43.2, ~/.local/share/opencode/opencode.db

1 failing, 1 warning(s)
```

It exits 1 when anything fails. `crew doctor --fix` applies the repairs crew owns (its own hooks, its own mailbox permissions, stale registry entries) and re-checks; it never edits a settings file it can't parse or a plugin it didn't generate. `crew doctor --json` prints `{"schema":1,"version":…,"ok":…,"checks":[{"id","label","status","detail","fix"}]}` for agents and scripts.
...
`crew install-hook` adds `crew --hook` to the same four lifecycle events in Claude Code and Codex (a plugin install registers them inside the plugin instead of user settings):
...
crew doctor                      # verify the wiring; --fix re-runs install-hook for you
CREW_NO_HOOK=1 npm i -g @0xmmo/crew   # install without postinstall touching hook settings
```
