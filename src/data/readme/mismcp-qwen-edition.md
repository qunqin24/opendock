...
> **qwen edition.** This fork is adapted for users of the **Qwen** model family
> (and other OpenAI-compatible backends with strict chat templates). It keeps
> exactly **one leading `system` message** in every request, fixing the
> `400 System message must be at the beginning.` error that Qwen models raise
> when a plugin appends a second system message. See
> [Qwen compatibility](#qwen-compatibility).

Distributed as a **git repository** (no npm package).

## Install

```bash
git clone https://github.com/mdementev/mismcp-qwen-edition.git
opencode plugin /path/to/mismcp-qwen-edition
```

or add a local path to `opencode.json` (absolute, or relative to the project),
then restart opencode:

```jsonc
{
  "plugin": ["/path/to/mismcp-qwen-edition"]
}
```

You can also point directly at the plugin file with `"plugin": ["./plugin/mismcp.ts"]`,
or let opencode auto-discover it from the `plugin/`/`plugins/` directory of a
cloned project.

## Qwen compatibility

Qwen chat templates (vLLM / SGLang with the stock HF template, DashScope
compatible-mode, …) reject any `system` message that is not the **single** one
at position `0`:

```
OpenAIException / 400 BadRequestError: System message must be at the beginning.
```

opencode itself serialises every entry of the `experimental.chat.system.transform`
hook output as a separate `role: "system"` message, so a bus plugin that appends
the roster ("Available agents to ask via …") would otherwise turn every request
into `[system, system, user, …]` and Qwen models would fail on the very first
message. This edition fixes it in the plugin (upstream: anomalyco/opencode#47003):

- `experimental.chat.system.transform` — coalesces all system fragments into a
  **single** leading system message (in-place, safe when other plugins also add
  to the system prompt).
- `experimental.chat.messages.transform` — defense-in-depth: merges any stray
  `system`/`developer` messages in the conversation history into one entry at
  `messages[0]` (no-op for a normal user/assistant history).

Remove the plugin and the error comes back, because the second system message is
gone — it is not a Qwen-side workaround around broken prompts, it is the plugin
no longer introducing the offending message.
...
- Windows: [agents-startup-scripts-demo/windows](https://github.com/mdementev/mismcp-qwen-edition/tree/main/agents-startup-scripts-demo/windows) — `tester.bat`, `analyst.bat`, etc.
- macOS: [agents-startup-scripts-demo/mac](https://github.com/mdementev/mismcp-qwen-edition/tree/main/agents-startup-scripts-demo/mac) — `tester.command`, `analyst.command`, etc.
