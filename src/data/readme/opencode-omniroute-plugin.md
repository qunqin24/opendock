...
Every field is optional. **The picker shows only active providers' active models by default** (`enabledOnly` + `activeOnly` both ON). Set either to `false` to restore the pre-v0.2.26 full-catalog behaviour.
...
| `enabledOnly`         | `boolean` | `true`  | **Default ON.** Read `/api/providers` and filter the catalog to providers whose connections have `isActive: true`. Unlike `usableOnly`, this **ignores `testStatus` entirely** — providers that are enabled but failing connection tests still surface their models. Subtract-filter semantics: synthetic prefixes like `agentrouter/*` (unknown to BOTH the connections table and the enrichment map) pass through; known-but-disabled prefixes are dropped. Soft-fails on fetch error/empty connections — never hides the whole catalog. Combos always pass through (they have no enable/disable in OmniRoute's dashboard). Set `enabledOnly: false` to restore full-catalog behavior. |
| `activeOnly`          | `boolean` | `true`  | **Default ON.** Read `/api/models` and filter the catalog to models whose `available !== false`. Models from disabled providers are already dropped by `enabledOnly`; this further drops models the operator has not explicitly pinned to the dashboard's active list. Soft-fails to no-filter on fetch error/empty response. Known limitation: `/api/models` IDs sometimes diverge from `/v1/models` IDs (upstream issue diegosouzapw/OmniRoute#10615) — models whose IDs don't match exactly pass through unfiltered. Combos always pass through. Set `activeOnly: false` to disable. |
