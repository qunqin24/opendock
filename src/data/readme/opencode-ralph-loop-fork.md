...
> Fork of [charfeng1/opencode-ralph-loop](https://github.com/charfeng1/opencode-ralph-loop). Modifications: commands now use the `orchestrator` agent (was `build`); the `help` command is renamed to `/ralph-help` to avoid shadowing opencode's built-in `/help`.
...
```json
{
  "plugin": ["opencode-ralph-loop-fork"]
}
```
...
- **Commands**: `/ralph-loop`, `/cancel-ralph`, and `/ralph-help`
